...
use d3d11_win::{D3D11_USAGE, D3D11_CPU_ACCESS_FLAG, D3D_DRIVER_TYPE, D3D_FEATURE_LEVEL,
                D3D11CreateDevice};
...
impl<T: COMInterface> UniqueCOMPtr<T> {
    /// Construct a new unique COM pointer from a pointer to a COM interface. It is the users
    /// responsibility to guarantee that no copies of the pointer exists beforehand
...
fn d3d11_create_device(adapter: &mut IDXGIAdapter)
                       -> (UniqueCOMPtr<ID3D11Device>,
                           UniqueCOMPtr<ID3D11DeviceContext>) {
    unsafe {
...
fn get_adapter_outputs(adapter: &mut IDXGIAdapter1) -> Vec<UniqueCOMPtr<IDXGIOutput>> {
        ...
        unsafe {
            ...
            if hr_failed(adapter.EnumOutputs(i, &mut output)) {
                ...
                if out_desc.AttachedToDesktop != 0 {
                    outputs.push(UniqueCOMPtr::new(output))
                } else {
                    break;
                }
            }
...
fn get_capture_source(output_dups: Vec<(UniqueCOMPtr<IDXGIOutputDuplication>,
                                        UniqueCOMPtr<IDXGIOutput1>)>,
                      cs_index: usize)
                      -> Option<(UniqueCOMPtr<IDXGIOutputDuplication>,
                                 UniqueCOMPtr<IDXGIOutput1>)> {
    if cs_index == 0 {
...
fn duplicate_outputs(mut device: UniqueCOMPtr<ID3D11Device>,
                     outputs: Vec<UniqueCOMPtr<IDXGIOutput>>)
                     -> Result<(UniqueCOMPtr<ID3D11Device>,
                                Vec<(UniqueCOMPtr<IDXGIOutputDuplication>,
                                     UniqueCOMPtr<IDXGIOutput1>)>),
                               HRESULT> {
    ...
    for mut output in outputs.into_iter()
        ...
        device = dxgi_device.query_interface().unwrap();
...
impl DuplicatedOutput {
    ...
    fn capture_frame_to_surface(&mut self,
                                timeout_ms: u32)
        ...
        let mut frame_texture = frame_resource.query_interface::<ID3D11Texture2D>().unwrap();
...
impl DXGIManager {
    ...
    pub fn acquire_output_duplication(&mut self) -> Result<(), ()> {
        ...
        for (outputs, mut adapter) in (0..)
                                          .map(|i| {
                                              let mut adapter = ptr::null_mut();
                                              unsafe {
                                                  if factory.EnumAdapters1(i, &mut adapter) !=
                                          ...
                                          .map(Option::unwrap)
                                          .map(|mut adapter| {
                                              (get_adapter_outputs(&mut adapter), adapter)
                                          })
            ...
            let (d3d11_device, output_duplications) = try!(duplicate_outputs(d3d11_device,
                                                                             outputs)
                                                               .map_err(|_| ()));
